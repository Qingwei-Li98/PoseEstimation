...
class Human36M(Dataset):
    def __init__(self, root,pose_root, activities=None,
             actors=None, sequence_len=20, random_seed=None, transforms=transforms.ToTensor(),is_train=True, subsampled_size=None):
        """
                ...
                for seq in activity_sequences:
                    ...
                    if len(frames)>1:
                        ...
                        if pose_num<frame_len: 
                            ...
                            total_len=frame_len
                        t = total_len//self.seq_len
                        frame2_num=rnd.randint(1,frame_len)
                        for i in range(t):
                            self.sequences.append({ 'actor': actor, # str
                                                   'activity_sequence': seq, # str
                                                   'frames': frames[self.seq_len*i:self.seq_len*(i+1)], # list of int
    ...
    def get_pose2D(self, sequence,size):
        ...
        pose_tensor= torch.FloatTensor(pose_array).permute(1,2,0)
        pose_tensor[:,0,:]=pose_tensor[:,0,:]*256/size[0]
        pose_tensor[:,1,:]=pose_tensor[:,1,:]*256/size[1]

        return pose_tensor[joints,:,:]
